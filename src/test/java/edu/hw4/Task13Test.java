...
public class Task13Test {
    static Arguments[] source() {
        ...
        Animal pauk = new Animal("p a u k", Animal.Type.SPIDER, Animal.Sex.M, 1, 4, 1, true);
        return new Arguments[] {
            Arguments.of(List.of(murka, bobik, sharik, bigdog1, bigdog2, pauk), List.of(bigdog1, bigdog2, pauk)),
            Arguments.of(List.of(murka, bobik, sharik, pauk), List.of(pauk)),
            Arguments.of(List.of(bigdog1), List.of(bigdog1)),
            Arguments.of(List.of(murka, sharik), List.of()),
    ...
    @ParameterizedTest
    @MethodSource("source")
    @DisplayName("Тест задания 13")
    void TaskTest(Collection<Animal> collection, List<Animal> expected) {
        // given
        // input

        // when
        List<Animal> list = AnimalMethods.task13(collection);
